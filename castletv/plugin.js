(function() {
    ...
    // Privacy: randomize device fingerprint fields per session so requests
    // cannot be correlated across sessions via static identifiers.
    var ANDROID_VERSIONS = ["11", "12", "13", "14"];
    ...
    // Privacy: enforce HTTPS — reject any manifest baseUrl that isn't https://
    function getApiBase() {
    ...
    // Privacy: no Referer header sent
    ...
    async function getDecryptedJson(url) {
        ...
        if (directPayload && typeof directPayload === "object") {
            return { securityKey: securityKey, data: directPayload };
        }
        ...
        return { securityKey: securityKey, data: parsed };
    }
    ...
    async function postDecryptedJson(url, body, securityKey) {
        ...
        var directPayload = safeJsonParse(encryptedPayload);
        if (directPayload && typeof directPayload === "object") {
            return { securityKey: payloadKey, data: directPayload };
        }
        ...
        return { securityKey: payloadKey, data: parsed };
    }
    ...
    async function collectEpisodes(details, securityKey, fallbackMovieId) {
        ...
        if (details && details.seasons && details.seasons.length > 1) {
            for (var i = 0; i < details.seasons.length; i++) {
                ...
                // Privacy: jitter between season fetches to break timing fingerprinting
                if (i > 0) await randomDelay(80, 300);

                try {
                    var seasonData = await fetchMovieDetails(String(season.movieId));
        ...
        return { securityKey: responseKey, episodes: allEpisodes };
    }
    ...
    async function resolveVideoPayload(movieId, episodeId, languageId, resolution, securityKey) {
        ...
        var body = {
            ...
            clientType: CLIENT_TYPE,
            woolUser: SESSION_WOOL_USER,
            apkSignKey: APK_SIGN_KEY,
            androidVersion: SESSION_ANDROID_VERSION,
            movieId: String(movieId),
            episodeId: String(episodeId),
            isNewUser: SESSION_IS_NEW_USER,
        ...
        try {
            ...
            if (!hasIndividualVideo) {
                ...
                for (var r = 0; r < resolutions.length; r++) {
                    // Privacy: jitter between resolution requests
                    if (r > 0) await randomDelay(50, 200);
                ...
                for (var t = 0; t < availableTracks.length; t++) {
                    ...
                    for (var x = 0; x < resolutions.length; x++) {
                        // Privacy: jitter between per-language resolution requests
                        if (x > 0 || t > 0) await randomDelay(50, 200);
